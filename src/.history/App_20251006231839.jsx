...
// Combine per-patch player blocks into one (overall + by region)
function combinePlayersAcrossPatches(byPatch, roleKey) {
  // overall
  const overall = new Map();              // key -> { puuid?, name, tag, games, wins }
  // per-region
  const regions = new Map();              // region -> Map(key -> stats)

  const bump = (dstMap, row) => {
    if (!row) return;
    const key = row.puuid || `${row.name || ""}#${row.tag || ""}`;
    let v = dstMap.get(key);
    if (!v) {
      v = { puuid: row.puuid || null, name: row.name || null, tag: row.tag || null, games: 0, wins: 0 };
      dstMap.set(key, v);
    }
    const g = Number(row.games) || 0;
    const w = (row.wins != null) ? Number(row.wins)
            : (row.winRate != null ? Number(row.winRate) * g : 0);
    v.games += g;
    v.wins  += w;
  };

  for (const p of Object.keys(byPatch || {})) {
    const block = byPatch?.[p]?.[roleKey] || byPatch?.[p]?.ALL;
    if (!block) continue;

    // union of listed players (we only have Top5 per patch available)
    (block.bestTop5 || []).forEach(r => bump(overall, r));
    (block.mostByGamesTop5 || []).forEach(r => bump(overall, r));

    // regions (if exporter wrote them)
    const R = block.regions || {};
    for (const code of Object.keys(R)) {
      const map = regions.get(code) || new Map();
      (R[code].bestTop5 || []).forEach(r => bump(map, r));
      (R[code].mostByGamesTop5 || []).forEach(r => bump(map, r));
      regions.set(code, map);
    }
  }
  ...
  const finalize = (map) => {
    ...
    const totalG = arr.reduce((s, x) => s + (x.games || 0), 0);
    const totalW = arr.reduce((s, x) => s + (x.wins  || 0), 0);
    const mu = totalG ? (totalW / totalG) : 0.5;
    const K = 50;

    arr.forEach(x => {
      x.winRate = x.games ? (x.wins / x.games) : null;
      x.score   = ((x.wins || 0) + K * mu) / ((x.games || 0) + K); // same shape as exporter’s bayes_score
    });

    const bestTop5       = [...arr].sort((a,b) =>
      (b.score ?? 0) - (a.score ?? 0) || (b.games||0) - (a.games||0)).slice(0,5);

    const mostByGamesTop5 = [...arr].sort((a,b) =>
      (b.games||0) - (a.games||0) || (b.winRate??0) - (a.winRate??0)).slice(0,5);

    // slim to the fields the UI expects
    const slim = x => ({ name: x.name || "—", tag: x.tag || "—", games: x.games || 0, winRate: x.winRate ?? null });
    return { bestTop5: bestTop5.map(slim), mostByGamesTop5: mostByGamesTop5.map(slim) };
  };

  const overallOut = finalize(overall);

  // regions out
  const regionsOut = {};
  for (const [code, m] of regions.entries()) {
    const r = finalize(m);
    if ((r.bestTop5.length + r.mostByGamesTop5.length) > 0) regionsOut[code] = r;
  }
  if (Object.keys(regionsOut).length) overallOut.regions = regionsOut;
...
async function loadChampionPlayers(BASE, slug, patch, role) {
  ...
  try {
    ...
    if (patch === "__ALL__") {
      return combinePlayersAcrossPatches(byPatch, roleKey);
    }
