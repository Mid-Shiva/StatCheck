...
function BestPlayersTables({ BASE, slug, patch, role }) {
  ...
  const [region, setRegion] = useState(null); // null = overall
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!ignore) {
        setBlock(data);
        setStatus("done");
        setRegion(null); // reset when inputs change
      }
    })();
    return () => { ignore = true; };
  ...
  if (status === "loading") {
    return <div className="mt-6"><div className="h-24 rounded border border-neutral-800 bg-neutral-900 animate-pulse" /></div>;
  }
  if (!block) return null;

  const regionsObj = block.regions || null;
  const regionKeys = regionsObj ? Object.keys(regionsObj).sort() : [];

  const current = region && regionsObj ? regionsObj[region] : block;
  const best = current?.bestTop5 || [];
  const most = current?.mostByGamesTop5 || [];

  if (best.length === 0 && most.length === 0) return null;
  ...
  const RegionTabs = () => {
    ...
    return (
      <div className="flex flex-wrap gap-2 mb-2">
        <button
          className={`px-2 py-1 rounded border text-xs ${region===null ? "bg-blue-600 text-white border-blue-600" : "bg-neutral-900 border-neutral-800 text-neutral-200"}`}
        ...
        {regionKeys.map(r => (
          <button
            key={r}
            className={`px-2 py-1 rounded border text-xs ${region===r ? "bg-blue-600 text-white border-blue-600" : "bg-neutral-900 border-neutral-800 text-neutral-200"}`}
            onClick={() => setRegion(r)}
          >
            {r}
          </button>
  ...
  const RankBadge = ({ i }) => {
    if (i === 0) return <span className="px-1.5 py-0.5 text-[10px] rounded-full bg-amber-500/15 text-amber-300 border border-amber-400/40">🥇 1</span>;
    if (i === 1) return <span className="px-1.5 py-0.5 text-[10px] rounded-full bg-slate-400/15 text-slate-200 border border-slate-300/40">🥈 2</span>;
    if (i === 2) return <span className="px-1.5 py-0.5 text-[10px] rounded-full bg-amber-700/15 text-amber-200 border border-amber-600/40">🥉 3</span>;
    return (
      <span className="w-6 h-6 inline-grid place-items-center text-[11px] rounded-full bg-neutral-800 text-neutral-300 border border-neutral-700">
        {i + 1}
      </span>
    );
  };
  ...
  const WinBar = ({ wr }) => {
    if (wr == null) return <div className="h-2 rounded bg-neutral-800" />;
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-sm ring-1 ring-neutral-200/60 dark:ring-neutral-800/60">
      <div className="px-3 py-2 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between">
        <div className="text-sm font-semibold">{title}</div>
        <div className="text-xs text-neutral-500 dark:text-neutral-400">{rows.length} players</div>
      </div>

      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead className="sticky top-0 z-10 bg-neutral-50/80 dark:bg-neutral-800/60 backdrop-blur supports-[backdrop-filter]:bg-neutral-800/40">
          ...
          <tbody>
            {rows.map((r, i) => {
              const player = [r.name, r.tag].filter(Boolean).join(" # ") || "—";
              return (
                <MotionTableRow
                  key={`${player}_${i}`}
                  className="border-t border-neutral-200 dark:border-neutral-800
                ...
                >
                  ...
                  <td className="px-3 py-2 text-right tabular-nums">{fmtInt(r.games)}</td>

                  <td className="px-3 py-2 text-right tabular-nums">
                    <div className="flex items-center gap-3 justify-end">
                      <span className="w-14 text-right">{fmtPct1(r.winRate)}</span>
                      <div className="w-28"><WinBar wr={r.winRate} /></div>
                    </div>
                  </td>
                </MotionTableRow>
              );
  ...
  const niceRegion = region ? ` — ${region}` : "";

  return (
    <div className="mt-6 space-y-3">
      <RegionTabs />
      <div className="grid gap-3 md:grid-cols-2">
        <Table rows={best} title={`Best ${slug} ${niceRegion}`} />
        <Table rows={most} title={`Most by Games ${slug} ${niceRegion}`} />
      </div>
    </div>
  );
}
...
function combineAcrossPatches(allData, role) {
  ...
  for (const p of Object.keys(allData)) {
    ...
    // games-weighted averages
    acc.avgGameLengthMin_sum += (r.avgGameLengthMin || 0) * g;
    acc.avgKills_sum         += (r.avgKills || 0) * g;
    acc.avgDeaths_sum        += (r.avgDeaths || 0) * g;
    acc.avgAssists_sum       += (r.avgAssists || 0) * g;
    acc.avgGoldDiffAt5_sum   += (r.avgGoldDiffAt5 || 0) * g;
    acc.avgGoldDiffAt10_sum  += (r.avgGoldDiffAt10 || 0) * g;
    acc.avgCsAt10_sum        += (r.avgCsAt10 || 0) * g;
    acc.avgCsDiffAt10_sum    += (r.avgCsDiffAt10 || 0) * g;
    acc.avgDPM_sum           += (r.avgDPM || 0) * g;
    acc.avgKP_sum            += (r.avgKP  || 0) * g;

    if (r.teamFBTowerRate != null) {
      acc.teamFBTowerRate_num += Number(r.teamFBTowerRate) * g;
      acc.teamFBTowerRate_den += g;
    }

    // rune paths (key on full path)
    for (const rp of (r.runePathsTop || [])) {
      const key = [
        rp.keystone, rp.rune1, rp.rune2, rp.rune3, rp.subRune1, rp.subRune2
      ].map(x => x || "").join("|");
      const cur = acc.runePathsMap.get(key) || { games: 0, wins: 0 };
      cur.games += Number(rp.games) || 0;
      cur.wins  += Number(rp.wins)  || 0;
      acc.runePathsMap.set(key, cur);
    }

    // mastery buckets (carry wins and games; re-average mastery%)
    for (const k of Object.keys(acc.mastery)) {
      const m = r?.mastery?.[k];
      if (!m) continue;
      const mg = Number(m.games) || 0;
      const mw = Number(m.wins)  || 0;
      acc.mastery[k].games += mg;
      acc.mastery[k].wins  += mw;
      if (k !== "BelowTop5Mastery" && m.avgChampMasteryPer != null) {
        acc.mastery[k].sumAvg += Number(m.avgChampMasteryPer) * mg;
        acc.mastery[k].denAvg += mg;
      }
    }

    // shards 3×3 grid
    const grid = Array.isArray(r.shardsGrid) ? r.shardsGrid : [];
    for (let si = 0; si < 3; si++) {
      const options = grid[si]?.options || [];
      for (const opt of options) {
        const row = (acc.shardsGrid[si][opt.name] ||= { games: 0, wins: 0 });
        row.games += Number(opt.games) || 0;
        row.wins  += Number(opt.wins)  || 0;
      }
    }

    // opponents / matchups
    for (const o of (r.opponents || r.matchups || [])) {
      const name = o.opponentChamp || o.enemy || o.name;
      if (!name) continue;
      const row = (acc.opponents[name] ||= { games: 0, wins: 0 });
      row.games += Number(o.games) || 0;
      row.wins  += Number(o.wins)  || 0;
    }

    // summoner spell combos (carry avg uses via sum * games)
    for (const c of (r.summonerCombosTop || [])) {
      const key = (c.spells || []).join("|");
      const row = (acc.summonerCombos[key] ||= { games: 0, wins: 0, sumUses: [0,0,0] });
      const cg  = Number(c.games) || 0;
      row.games += cg;
      row.wins  += Number(c.wins) || 0;
      const uses = c.avgUses || c.uses || [];
      for (let i = 0; i < row.sumUses.length; i++) {
        row.sumUses[i] += (Number(uses[i]) || 0) * cg;
      }
    }

    // items: starter, support, boots(+tiered), first10, legendary[], first_item
    const bump = (map, key, deltaGames, deltaWins) => {
      const r = (map[key] ||= { games: 0, wins: 0 });
      r.games += deltaGames; r.wins += deltaWins;
    };

    const addOptions = (target, options) => {
      for (const it of (options || [])) {
        const g = Number(it.games) || 0;
        const w = Number(it.wins)  || 0;
        bump(target, it.item || it.name || it.id, g, w);
      }
    };

    addOptions(acc.items.starter,  r?.items?.starter?.options);
    addOptions(acc.items.support,  r?.items?.support?.options);
    addOptions(acc.items.boots,    r?.items?.boots?.options);
    addOptions(acc.items.boots_t1, r?.items?.bootsTiered?.tier1);
    addOptions(acc.items.boots_t2, r?.items?.bootsTiered?.tier2);
    addOptions(acc.items.boots_t3, r?.items?.bootsTiered?.tier3);
    addOptions(acc.items.first10,  r?.items?.first10);

    // Magical Footwear timing
    if (r?.items?.boots?.footwear) {
      const fw = r.items.boots.footwear;
      const fg = Number(fw.share || 0) * g;
      acc.items.footwear_games     += fg;
      acc.items.footwear_sum_time  += (Number(fw.avg_time_s) || 0) * fg;
    }

    // legendary slots
    (r?.items?.legendary || []).forEach((slot, i) => {
      if (!acc.items.legendary[i]) acc.items.legendary[i] = {};
      addOptions(acc.items.legendary[i], slot.options);
    });

    // first item comparison blob (carry raw) if present
    if (r?.items?.first_item) {
      for (const [name, v] of Object.entries(r.items.first_item)) {
        const dst = (acc.items.first_item[name] ||= {});
        for (const [k, val] of Object.entries(v)) {
          dst[k] = (dst[k] || 0) + (Number(val) || 0);
        }
      }
    }
