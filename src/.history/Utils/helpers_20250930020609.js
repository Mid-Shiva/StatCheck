...
// Return the share (0..1) of games from "BestMastery" players in the current role.
// Accepts either a "cur" object with { mastery, games } or a mastery object directly.
// Optionally, pass totalGames as second arg as a fallback.
export function getBestMasteryShare(source, totalGamesMaybe) {
  if (!source) return null;

  // Support both shapes:
  const cur = source;
  const mastery = (cur && cur.mastery) ? cur.mastery : source;
  ...
  // 1) direct fraction if present
  if (typeof bm.share === "number" && Number.isFinite(bm.share)) {
    return bm.share;
  }

  // 2) percent fields (convert to fraction)
  if (typeof bm.pct === "number" && Number.isFinite(bm.pct)) {
    return bm.pct > 1 ? bm.pct / 100 : bm.pct;
  }
  if (typeof bm.percent === "number" && Number.isFinite(bm.percent)) {
    return bm.percent > 1 ? bm.percent / 100 : bm.percent;
  }

  // 3) derive from counts if available
  const totalGames =
    (typeof totalGamesMaybe === "number" && totalGamesMaybe > 0)
      ? totalGamesMaybe
      : (typeof cur?.games === "number" ? cur.games : null);

  if (typeof bm.games === "number" && totalGames > 0) {
    return bm.games / totalGames;
  }
